...
dat$q9_baseline[dat$q9_baseline != 'Tigre'] <- 0
dat$q9_baseline[dat$q9_baseline == 'Tigre'] <- 1
...
columns <- c('ethnic_group_baseline', 'federal_states_baseline', 'contact_govt_baseline', 'ethiopian_people_baseline', 'tigray', 'discrimination')
...
m2 <- lm(national_index ~ ethnic_index * tigray, data = dat_z)
...
## plot non-Tigrayans 
plot(dat_z$ethnic_index[dat$tigray == 0], dat_z$national_index[dat$tigray == 0],
     main = 'National vs Ethnic Identification',
     xlab = 'Ethnic Identification',
     ylab = 'National Identification',
     pch = 16)
## add regression line
abline(m1)
## add points for Tigray
points(dat_z$ethnic_index[dat$tigray == 1], dat_z$national_index[dat$tigray == 1],
       col = 'darkred')

sum(is.na(dat_z$ethnic_index))
summary(dat$tigray)
```

### Conclusion

This also implies that the framing of survey questions such as that mentioned in the introduction fail to adequately measure ethnic identification and national identitification. They cannot account for people who identify with both their ethnicity and their nationality. Hence, ...
