...
$EthnicID$ is the averaged z-score of two measures of ethnic identification (see the Appendix for question wording), $NationalID$ the averaged z-score of two measures of national identification, and $\epsilon$ the error term.

Additionally, I introduce an interaction term ...

$$ NationalID_i =\beta_1 * EthnicID_i + \beta_2 * Discrimination_i + \beta_3 * (EthnicID_i * Discrimination_i) + \epsilon_i $$

This might also spark some ideas about how to foster dual-identification in fractured societies.

Does it make sense to have controls and interaction in separate models?
...
models[['Bivariate w/ Control Variables']] <- lm(national_index ~ ethnic_index + contacts + urban, data = dat_z)
